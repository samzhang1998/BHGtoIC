...
//   navBar.value.title = t('setting.setting20')
// }
...
const handleToChange = (str: string) => {
  const path = str === 'email' ? '/pages/user/resetPin?type=email' : '/pages/user/resetPin?type=phone'
  uni.redirectTo({
    url: path
  })
}
...
<template>
  <view class="login">
    ...
    <view class="content">
      ...
      <view class="text">{{ $t('setting.setting26') }}</view>
    </view>
