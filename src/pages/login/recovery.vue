...
async function sendCodeForEmail() {
  ...
  const [e, r] = await api.resetPasswordCodeSend(params)
  if (!e && r) {
    if (r?.success) {
      uni.showToast({
        icon: 'none',
...
async function sendCodeForPhone() {
  ...
  const [e, r] = await api.resetPasswordCodeMsgSend(params)
  if (!e && r) {
    if (r?.success) {
      uni.showToast({
        icon: 'none',
...
async function resetPassword() {
  ...
  const [e, r] = await api.resetPassword(params)
  if (!e && r) {
    if (r?.success) {
      uni.showToast({
        icon: 'none',
...
<template>
  <view class="recovery">
    ...
    <view class="info" v-if="pageType === 2">
      <view class="title">{{ $t('login.new') }}</view>
      <view class="form">
        <uni-forms ref="loginForm" :rules="rules" :modelValue="loginFormData" label-position="top">
          <uni-forms-item :label="$t('login.newPassword') + ':'" name="password">
            <uni-easyinput v-model="loginFormData.password" type="password" placeholder="" />
          </uni-forms-item>
          <uni-forms-item :label="$t('login.confirmPassword') + ':'" type="password" name="password2">
            <uni-easyinput v-model="loginFormData.password2" placeholder="" type="password" />
          </uni-forms-item>
        </uni-forms>
        <view class="login-btn" @click="handleConfirm">{{ $t('login.confirm') }}</view>
      </view>
    </view>
  </view>
