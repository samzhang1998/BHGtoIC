...
async function sendCode() {
  ...
  const [e, r] = await api.registerCodeSend(params)
...
async function resetPassword() {
  let params = {
    ...
    confirmPassword: loginFormData.value.password2,
    verificationCode: loginFormData.value.code
  }
  const [e, r] = await api.registerClient(params)
  if (!e && r) {
    if (r?.success) {
      uni.showToast({
        icon: 'none',
...
<template>
  <view class="recovery">
    ...
    <view class="info" v-if="pageType === 2">
      <view class="title">{{ $t('login.new1') }}</view>
      <view class="form">
        <uni-forms ref="loginForm" :rules="rules" :modelValue="loginFormData" label-position="top">
          <uni-forms-item :label="$t('login.newPassword1') + ':'" name="password">
            <uni-easyinput v-model="loginFormData.password" type="password" placeholder="" />
          </uni-forms-item>
          <uni-forms-item :label="$t('login.confirmPassword1') + ':'" type="password" name="password2">
            <uni-easyinput v-model="loginFormData.password2" placeholder="" type="password" />
          </uni-forms-item>
        </uni-forms>
        <view class="login-btn" @click="handleConfirm">{{ $t('login.login4') }}</view>
      </view>
    </view>
  </view>
  <uni-popup ref="popup" :is-mask-click="false" type="bottom">
    <view class="pop">
      ...
      <view class="text">
        {{ $t('login.resend') }}
        <text class="text-BE9670">{{ $t('login.resend2') }}</text>
      </view>
